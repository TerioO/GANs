...
onMounted(() => { if (props.data) createCanvasImageData() })
watch(() => props.data, () => createCanvasImageData())
watch(() => canvasModal.value, () => {
    copyCanvasToModal()
})
...
<template>
    <form class="flex flex-col">
        ...
        <div v-show="!loading && data" class="mt-6">
            <canvas ref="canvas" class="border-2 border-emerald-500 w-fit h-fit cursor-pointer"
                @click="toggleModal"></canvas>
        </div>
        <Dialog v-model:visible="modalVisible" modal header="Image" :style="{ width: '90vw', height: '90vh' }" :pt="{
            content: {
                style: {
                    height: '100%'
                }
            ...
            <div class="h-full flex flex-col">
                ...
                <div class="flex-1 overflow-auto flex items-center justify-center">
                    <canvas ref="canvasModal" :style="{ transform: `scale(${canvasScale})` }"></canvas>
                </div>
            </div>
        </Dialog>
    </form>
