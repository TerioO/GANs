        ...
        for epoch in tqdm(range(epochs)):
            ...
            for _, (img, labels) in enumerate(dataloader_train):
                ...
                disc_loss_fake = criterion(
                    y_pred_fake,
                    helpers.get_GAN_labels(curr_step, [150, 300, 550, 700], [(0, 0), (0.0, 0.05), (0.0, 0.1), (0, 0.15)], y_pred_fake.shape, ones=False).to(device) 
                )
                disc_loss_real = criterion(
                    y_pred_real, 
                    helpers.get_GAN_labels(curr_step, [150, 300, 550, 700], [(1, 1), (0.95, 1), (0.9, 1), (1, 1)], y_pred_real.shape, ones=True).to(device)
                )
...
# [MAIN PROGRAM] -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def main():
    # tensorboard --samples_per_plugin "images=1000,scalars=5000" --logdir="./Prototypes/models state_dict/CDCGAN_Human_faces_v1/tensorboard"
    os.system("cls")
