...
class Generator(nn.Module):
    def __init__(self, in_features: int, hidden_units: int, out_features: int,  out_channels: int, num_classes: int, img_size: int):
        super().__init__()
    ...
    def forward(self, x, labels):
        embedding = self.embedding(labels)              # [labels.shape, num_classes] = [N, 3]
        embedding = embedding.unsqueeze(2).unsqueeze(3) # [N, 3, 1, 1]
        embedding = embedding.expand(-1, -1, self.img_size, self.img_size)
        input = torch.cat([x, embedding], dim=1)        # [N, input_channels + 3, H, W]
        output = self.gen(input)                        # [N, 3, H, W] 
        output = output.view(x.shape[0], self.out_channels, self.img_size, self.img_size)
        return output 
    
# Gradient Penalty    
def GP(disc: nn.Module, labels: torch.Tensor, img_real: torch.Tensor, img_fake: torch.Tensor, device: str):
    N = img_real.shape[0]
    epsilon = torch.rand(N, 1, 1, 1, device=device, requires_grad=True).to(device)

    mixed_images = img_real*epsilon + img_fake*(1-epsilon)
    mixed_scores = disc(mixed_images, labels)
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for _, (img, labels) in enumerate(dataloader_train):
                ...
                for i in range(critic_iter):
                    ...
                    img_fake = gen(noise, labels_fake)
                    
                    y_pred_fake = disc(img_fake, labels_fake)
                    y_pred_real = disc(img, labels_real)
                    gp = GP(disc, labels_fake, img, img_fake, device)
...
# [MAIN PROGRAM] -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def main():
    # tensorboard --samples_per_plugin "images=1000,scalars=5000" --logdir="./Prototypes/models state_dict/WGP_CDCGAN_Animal_Faces_v2/tensorboard"
    os.system("cls")

    version = 2
    filenames: IFilenames = {
        "dir": f"WGP_CDCGAN_Animal_Faces_v{version}",
        "generator": f"gen",
        "discriminator": f"disc",
        "gan": f"gan",
        "tensorboard": helpers.get_tensorboard_dir(f"WGP_CDCGAN_Animal_Faces_v{version}")
    }
    ...
    gen_0 = Generator(in_features=(len(train.classes)+3)*img_size*img_size, 
                      hidden_units=512, 
                      out_features=3*img_size*img_size, out_channels=3, 
    ...
    def view_result_images(gen: nn.Module,
        ...
        labels = torch.randint(0, gen.num_classes, [N], dtype=torch.int)
        labels = torch.IntTensor(labels).to(device) # [N]
    ...
    view_result_images(gen_0, disc_0, 36, 9)
    ...
    def test_gan(gen: nn.Module, disc: nn.Module):
        ...
        img_fake_pred = disc(img_fake, labels)
