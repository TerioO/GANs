...
class Discriminator(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        embedding = embedding.expand(-1, -1, self.img_size, self.img_size) # [N, 2, H, W]
        input = torch.cat([x, embedding], 1)            # [N, input_channels + 2, H, W]
    ...
    try:
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for _, (img, labels) in enumerate(dataloader_train):
                ...
                penalty = min(0.2, 0.02 * json_log["progress"]["disc_lvl"]) if json_log["progress"]["disc_lvl"] > 0 else 0       
                disc_loss_fake = criterion(y_pred_fake, torch.zeros(y_pred_fake.shape).to(device) + penalty)
                disc_loss_real = criterion(y_pred_real, torch.ones(y_pred_real.shape).to(device) - penalty)
            ...
            # Update json_log losses:
            json_log["progress"]["gen_loss"].append(gen_epoch_loss)
            json_log["progress"]["disc_loss"].append(disc_epoch_loss)
            ...
            if global_step % json_log["progress"]["epochs_to_track"] == 0:
                ...
                for j in range(len(disc_losses) - 1):
                    if disc_losses[j+1] < disc_losses[j]: count_disc += 1
            
                if count_disc >= (json_log["progress"]["epochs_to_track"] - json_log["progress"]["tolerance"]):
                    json_log["progress"]["disc_lvl"] += 1
                elif json_log["progress"]["disc_lvl"] >= 1: json_log["progress"]["disc_lvl"] -= 1
                
                temp_lr = 0
                if json_log["progress"]["disc_lvl"] > 0:
                    for p in disc_optim.param_groups:
                        p["lr"] = json_log["disc_lr"] / (2**json_log["progress"]["disc_lvl"])
                        temp_lr = p["lr"]
                else:
                    for p in disc_optim.param_groups:
                        p["lr"] = json_log["disc_lr"]
                        temp_lr = p["lr"]
                        
                print(f"[DISC_LVL: {json_log["progress"]["disc_lvl"]}]")
                print(f"[NEW_DISC_LR: {temp_lr}]\n")
...
# [MAIN PROGRAM] -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def main():
    # tensorboard --samples_per_plugin "images=1000,scalars=5000" --logdir="./Prototypes/models state_dict/CDCGAN_Animal_Faces_v0/tensorboard"
    os.system("cls")
    ...
    gen_0 = Generator(input_channels=100, features=64 * 2**4, output_channels=3, num_classes=len(train.classes), img_size=img_size)
    disc_0 = Discriminator(input_channels=3, features=64 * 2**4, num_classes=len(train.classes), img_size=img_size)
