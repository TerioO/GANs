        ...
        for epoch in tqdm(range(epochs)):
            ...
            for _, (img, labels) in enumerate(dataloader_train):
                ...
                disc_loss_fake = criterion(
                    y_pred_fake,
                    helpers.get_GAN_labels(curr_step, [200, 400, 600], [(0, 0), (0, 0.5), (0, 0.1)], y_pred_fake.shape, ones=False).to(device) 
                )
                disc_loss_real = criterion(
                    y_pred_real, 
                    helpers.get_GAN_labels(curr_step, [200, 400, 600], [(1, 1), (0.95, 1), (1, 1)], y_pred_real.shape, ones=True).to(device)
                )
            ...
            if global_step % 5 == 0:
                ...
                score = helpers.metric_eval(gen, dataloader_train, device, 20, metric_type, cgan=False) # 640 real & 640 fake image
            ...
            # Change LRs:
            helpers.change_optim_lr(disc_optim, global_step, [200], [2e-4])
...
# [MAIN PROGRAM] -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def main():
    # tensorboard --samples_per_plugin "images=1000,scalars=5000" --logdir="./Prototypes/models state_dict/DCGAN_Simpsons_faces_v0/tensorboard"
    os.system("cls")

    version = 0
    filenames: IFilenames = {
        "dir": f"DCGAN_Simpsons_faces_v{version}",
        "generator": f"gen",
        "discriminator": f"disc",
        "gan": f"gan",
        "tensorboard": helpers.get_tensorboard_dir(f"DCGAN_Simpsons_faces_v{version}")
    }
    ...
    gen_0 = Generator(input_channels=100, features=1024, output_channels=3, img_size=img_size)
    disc_0 = Discriminator(input_channels=3, features=1024, img_size=img_size)
