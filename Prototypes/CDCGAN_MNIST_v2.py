...
# [MAIN PROGRAM] -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def main():
    # tensorboard --samples_per_plugin "images=1000,scalars=5000" --logdir="./Prototypes/models state_dict/CDCGAN_MNIST_v2/tensorboard"
    os.system("cls")
    ...
    def get_GAN_score():
        score = helpers.metric_eval(gen_0, train_dataloader, device, 2, "IS")
    
        def view_fid_images(imgs: torch.Tensor, nrow=10, title="FID Images"):
            imgs_grid = torchvision.utils.make_grid(imgs, nrow=nrow)
            imgs_grid = torch.as_tensor(imgs_grid).permute(1, 2, 0).detach().cpu().numpy()
        ...
        print(f"Score: {score}")
        # view_fid_images(imgs_real, 40, "REAL IMAGES")
        # view_fid_images(imgs_fake, 40, "FAKE IMAGES")       
